...
% start time
xS = find(abs(acc_time-47.9031)<0.002); xS = xS(1);
xE = find(abs(acc_time-51.4569)<0.001); xE = xE(1);

xxS = find(abs(gyro_time-47.9031)<0.002); xxS = xxS(1);
xxE = find(abs(gyro_time-51.4569)<0.001); xxE = xxE(1);

xSS = find(abs(mag_time-47.9031)<0.011); xSS = xSS(1);
...
figure
subplot(311), plot(time(2:end-N), pos, 'LineWidth', 3); hold on; plot(time, PP1, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','SouthEast'); title('Position Along x Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([47.9408, 51.4569])
subplot(312), plot(time(2:end-N), vel, 'LineWidth', 2); hold on; plot(time, VV1, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth'); title('Velocity Along x Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([47.9408, 51.4569])
subplot(313), plot(time(1:end-N), x(3,1:end), 'LineWidth', 2); hold on; plot(time, AA1, 'LineWidth', 2); legend('Estimated Acceleration','Ground Truth'); title('Acceleration Along x Axis [m/s^2]'); xlabel('t [s]'); ylabel('Acceleration [m/s^2]'); grid on; grid minor;xlim([47.9408, 51.4569])
