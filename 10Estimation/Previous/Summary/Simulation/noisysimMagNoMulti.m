function [v, v2, r, r2, rdot, rdot2] = noisysimMagNoMulti(x,f,Gt,M,X,PT,GT,GR,R,sigma,index,k,z,z_prev,T,time1,multi)
...
% ++++++++++++++++++++++++++++++++++++++++++ Noise of Magnitude +++++++++++++++++++++++++++++++++++++++++++
pd = makedist('Rician','s',sqrt(H),'sigma',0.025*sqrt(H));
v = random(pd).^2;
...
r = ((2*PT*GT*GR*Gt^2*lambda1^2*X^2*M*R)/((4*pi)^2*(v^2)))^(1/2);
r2 = ((2*PT*GT*GR*Gt^2*lambda1^2*X^2*M*R)/((4*pi)^2*(H2^2)))^(1/2);

% % ============================================== Phase ====================================================

phi_mod1     = angle(sqrt((2*R*PT*GT*GR*Gt^2*lambda1^2*X^2*M)/(4*pi)^2*(mul1     + 0.2*mulSum3)^2));% + 0.2*mulSum4 + 0.2*mulSum5 + 0.1*mulSum6 + 0.1*mulSum7 +0.2*mulSum8)^2));%4*pi*sqrt((z(1,k)-x(1))^2+(z(2,k)-x(2))^2+(z(3,k)-x(3))^2)/lambda; %4*pi*sqrt((z(1,k)-x(1))^2+(z(2,k)-x(2))^2+(z(3,k)-x(3))^2)/lambda;%mod(4*pi*sqrt((z(1,k)     -x(1))^2+(z(2,k)     -x(2))^2)/lambda,2*pi);
phi_prev_mod = angle(sqrt((2*R*PT*GT*GR*Gt^2*lambda1^2*X^2*M)/(4*pi)^2*(mul_prev + 0.2*mulSum3_prev)^2));% + 0.2*mulSum4_prev + 0.2*mulSum5_prev + 0.1*mulSum6_prev + 0.1*mulSum7_prev +0.2*mulSum8_prev)^2));
