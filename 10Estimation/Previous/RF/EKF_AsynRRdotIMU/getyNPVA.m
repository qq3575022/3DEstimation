function [y, i, j, k, index, len] = getyNPVA(r_sim1, rdot_sim1, r_sim2, rdot_sim2, r_sim3, rdot_sim3, r_sim4, rdot_sim4, mag_data_x, gyro_data_x, mag_data_y, gyro_data_y, mag_data_z, gyro_data_z, acc_data_x, acc_data_y, acc_data_z, acc_time, gyro_time, mag_time, time, i, j, k, m)
% m = m
% i = i
% j = j
% k = k
...
if i <= length(acc_time) && j <= length(gyro_time) && k <= length(mag_time) && abs(time(m) - acc_time(i)) == 0 && abs(time(m) - gyro_time(j)) ==0 && abs(time(m) - mag_time(k)) ==0
  ...
  len = 9+8;
  y = NaN(9+8,1);
  y(1:9+8) = [r_sim1(m), rdot_sim1(m), r_sim2(m), rdot_sim2(m), r_sim3(m), rdot_sim3(m), r_sim4(m), rdot_sim4(m), mag_data_x(k), gyro_data_x(j), mag_data_y(k), gyro_data_y(j), mag_data_z(k), gyro_data_z(j), acc_data_x(i), acc_data_y(i), acc_data_z(i)];
  i = i + 1;
  j = j + 1;
  k = k + 1;
...
elseif i <= length(acc_time) && j <= length(gyro_time) && abs(time(m) - acc_time(i)) == 0 && abs(time(m) - gyro_time(j)) == 0
  ...
  len = 6+8;
  y = NaN(6+8,1);
  y(1:6+8) = [r_sim1(m), rdot_sim1(m), r_sim2(m), rdot_sim2(m), r_sim3(m), rdot_sim3(m), r_sim4(m), rdot_sim4(m),gyro_data_x(j), gyro_data_y(j), gyro_data_z(j), acc_data_x(i), acc_data_y(i), acc_data_z(i)];
  i = i + 1;
  j = j + 1;
...
elseif j <= length(gyro_time) && k <= length(mag_time) && abs(time(m) - gyro_time(j)) == 0 && abs(time(m) - mag_time(k)) == 0
  ...
  len = 6+8;
  y = NaN(6+8,1);
  y(1:6+8) = [r_sim1(m), rdot_sim1(m), r_sim2(m), rdot_sim2(m), r_sim3(m), rdot_sim3(m), r_sim4(m), rdot_sim4(m),mag_data_x(k), gyro_data_x(j), mag_data_y(k), gyro_data_y(j), mag_data_z(k), gyro_data_z(j)];
  j = j + 1;
  k = k + 1;
...
elseif i <= length(acc_time) && k <= length(mag_time) && abs(time(m) - acc_time(i)) == 0 && abs(time(m) - mag_time(k)) == 0
  ...
  len = 6+8;
  y = NaN(6+8,1);
  y(1:6+8) = [r_sim1(m), rdot_sim1(m), r_sim2(m), rdot_sim2(m), r_sim3(m), rdot_sim3(m), r_sim4(m), rdot_sim4(m),mag_data_x(k), mag_data_y(k), mag_data_z(k), acc_data_x(i), acc_data_y(i), acc_data_z(i)];
  i = i + 1;
  k = k + 1;
...
elseif i <= length(acc_time) && abs(time(m) - acc_time(i)) == 0
  ...
  len = 3+8;
  y = NaN(3+8,1);
  y(1:3+8) = [r_sim1(m), rdot_sim1(m), r_sim2(m), rdot_sim2(m), r_sim3(m), rdot_sim3(m), r_sim4(m), rdot_sim4(m),acc_data_x(i), acc_data_y(i), acc_data_z(i)];
  i = i + 1;
...
elseif j <= length(gyro_time) && abs(time(m) - gyro_time(j)) < 0.0001
  ...
  len = 3+8;
  y = NaN(3+8,1);
  y(1:3+8) = [r_sim1(m), rdot_sim1(m), r_sim2(m), rdot_sim2(m), r_sim3(m), rdot_sim3(m), r_sim4(m), rdot_sim4(m),gyro_data_x(j), gyro_data_y(j), gyro_data_z(j)];
  j = j + 1;
...
elseif k <= length(mag_time) && abs(time(m) - mag_time(k)) < 0.0001
  ...
  len = 3+8;
  y = NaN(3+8,1);
  y(1:3+8) = [r_sim1(m), rdot_sim1(m), r_sim2(m), rdot_sim2(m), r_sim3(m), rdot_sim3(m), r_sim4(m), rdot_sim4(m),mag_data_x(k), mag_data_y(k), mag_data_z(k)];
  k = k + 1;
