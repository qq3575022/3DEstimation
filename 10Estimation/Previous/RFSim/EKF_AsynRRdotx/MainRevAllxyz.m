...
for m = 1:1:length(time)
    ...
    [y, i, j, k, index] = getyNPVA(dmeasr1, dmeasrdot1, dmeasr2, dmeasrdot2, dmeasr3, dmeasrdot3, dmeasr4, dmeasrdot4, magx, gyrox, magy, gyroy, magz, gyroz, accx, accy, accz, accT, gyroT, magT, time, i, j, k, m);
...
subplot(313), plot(time, x(9,:), 'LineWidth', 2); hold on; plot(time, AA3, 'LineWidth', 2); legend('Estimated Acceleration','Ground Truth'); title('Acceleration Along z Axis [m/s^2]'); xlabel('t [s]'); ylabel('Acceleration [m/s^2]'); grid on; grid minor;xlim([107.99, 111.984])
