...
%
H1 = NaN(1,length(time));  H1_ = NaN(1,length(PP1));   phi1 = NaN(1,length(PP1));       phi_mu1 = NaN(1,length(PP1));
H2 = NaN(1,length(time));  H2_ = NaN(1,length(PP1));   phi2 = NaN(1,length(PP1));       phi_mu2 = NaN(1,length(PP1));
H3 = NaN(1,length(time));  H3_ = NaN(1,length(PP1));   phi3 = NaN(1,length(PP1));       phi_mu3 = NaN(1,length(PP1));
H4 = NaN(1,length(PP1));  H4_ = NaN(1,length(PP1));   phi4 = NaN(1,length(PP1));       phi_mu4 = NaN(1,length(PP1));
...
[H1(k+1),phi_mu1(k+1),r_sim1(k+1),r_sim1_(k+1),rdot_sim1(k+1),rdot_sim1_(k+1),diff1(k+1)] = noisysim(x1,f1,Gt,M,X,PT,GT,GR,R,sigma,1,k,z,z_prev,time(k+1)-time(k),multi);
[H2(k+1),phi_mu2(k+1),r_sim2(k+1),r_sim2_(k+1),rdot_sim2(k+1),rdot_sim2_(k+1),diff2(k+1)] = noisysim(x2,f2,Gt,M,X,PT,GT,GR,R,sigma,2,k,z,z_prev,time(k+1)-time(k),multi);
[H3(k+1),phi_mu3(k+1),r_sim3(k+1),r_sim3_(k+1),rdot_sim3(k+1),rdot_sim3_(k+1),diff3(k+1)] = noisysim(x3,f3,Gt,M,X,PT,GT,GR,R,sigma,3,k,z,z_prev,time(k+1)-time(k),multi);   
[H4(k+1),phi_mu4(k+1),r_sim4(k+1),r_sim4_(k+1),rdot_sim4(k+1),rdot_sim4_(k+1),diff4(k+1)] = noisysim(x4,f4,Gt,M,X,PT,GT,GR,R,sigma,4,k,z,z_prev,time(k+1)-time(k),multi);  
end
...
figure
subplot(8,1,1),plot(time, 0.2*r_sim1+1.4,'LineWidth',2);hold on;plot(time, r_sim1_,'LineWidth',3);legend('Simulated','Ground truth');title('2D Radial Distance $R_1$','interpreter','latex');ylabel('radius [m]');xlabel('t [s]')
subplot(8,1,2),plot(time, 0.0001*rdot_sim1,'-','LineWidth',2);hold on;plot(time, rdot_sim1_,'-','LineWidth',2);legend('Simulated','Ground truth');title('2D Radial Velocity $\dot R_1$','interpreter','latex');ylabel('radial velocity [m/s]');xlabel('t [s]')
subplot(8,1,3),plot(time, 0.2*r_sim2+1.4,'LineWidth',2);hold on;plot(time, r_sim2_,'LineWidth',3);legend('Simulated','Ground truth');title('2D Radial Distance $R_2$','interpreter','latex');ylabel('radius [m]');xlabel('t [s]')
subplot(8,1,4),plot(time, 0.0001*rdot_sim2,'-','LineWidth',2);hold on;plot(time, rdot_sim2_,'-','LineWidth',2);legend('Simulated','Ground truth');title('2D Radial Velocity $\dot R_2$','interpreter','latex');ylabel('radial velocity [m/s]');xlabel('t [s]')
subplot(8,1,5),plot(time, 0.2*r_sim3+1.4,'LineWidth',2);hold on;plot(time, r_sim3_,'LineWidth',3);legend('Simulated','Ground truth');title('2D Radial Distance $R_3$','interpreter','latex');ylabel('radius [m]');xlabel('t [s]')
subplot(8,1,6),plot(time, 0.0001*rdot_sim3,'-','LineWidth',2);hold on;plot(time, rdot_sim3_,'-','LineWidth',2);legend('Simulated','Ground truth');title('2D Radial Velocity $\dot R_3$','interpreter','latex');ylabel('radial velocity [m/s]');xlabel('t [s]')
subplot(8,1,7),plot(time, 0.2*r_sim4+1.4,'LineWidth',2);hold on;plot(time, r_sim4_,'LineWidth',3);legend('Simulated','Ground truth');title('2D Radial Distance $R_4$','interpreter','latex');ylabel('radius [m]');xlabel('t [s]')
subplot(8,1,8),plot(time, 0.0001*rdot_sim4,'-','LineWidth',2);hold on;plot(time, rdot_sim4_,'-','LineWidth',2);legend('Simulated','Ground truth');title('2D Radial Velocity $\dot R_4$','interpreter','latex');ylabel('radial velocity [m/s]');xlabel('t [s]')
...
figure
subplot(411), plot(time,  r_sim1, 'LineWidth', 1), hold on, plot(time, r_sim1_, 'LineWidth', 1), title('Radial Distance $r1$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([107.99, 111.984]);
subplot(412), plot(time,  r_sim2, 'LineWidth', 1), hold on, plot(time, r_sim2_, 'LineWidth', 1),title('Radial Distance $r2$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([107.99, 111.984]);
subplot(413), plot(time,  r_sim3, 'LineWidth', 1), hold on, plot(time, r_sim3_, 'LineWidth', 1),title('Radial Distance $r3$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([107.99, 111.984]);
subplot(414), plot(time,  r_sim4, 'LineWidth', 1), hold on, plot(time, r_sim4_, 'LineWidth', 1),title('Radial Distance $r4$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([107.99, 111.984]);



figure
subplot(411), plot(time, rdot_sim1); hold on; plot(time, rdot_sim1_, 'LineWidth', 1), title('Radial Velocity $\dot r_1$','interpreter','latex'); xlabel('time[s]'), ylabel('Velocity [m/s]','interpreter','latex'), legend('measurement','ground truth'), xlim([107.99, 111.984]);
subplot(412), plot(time, rdot_sim2); hold on; plot(time, rdot_sim2_, 'LineWidth', 1), title('Radial Velocity $\dot r_2$','interpreter','latex'); xlabel('time[s]'), ylabel('Velocity [m/s]','interpreter','latex'), legend('measurement','ground truth'), xlim([107.99, 111.984]);
subplot(413), plot(time, rdot_sim3); hold on; plot(time, rdot_sim3_, 'LineWidth', 1), title('Radial Velocity $\dot r_3$','interpreter','latex'); xlabel('time[s]'), ylabel('Velocity [m/s]','interpreter','latex'), legend('measurement','ground truth','Location','SouthEast'), xlim([107.99, 111.984]);
subplot(414), plot(time, rdot_sim4); hold on; plot(time, rdot_sim4_, 'LineWidth', 1), title('Radial Velocity $\dot r_4$','interpreter','latex'); xlabel('time[s]'), ylabel('Velocity [m/s]','interpreter','latex'), legend('measurement','ground truth'), xlim([107.99, 111.984]);
...
for m = 1:1:length(time)
    ...
    [y, i, j, k, index, len] = getyNPVA(r_sim1, rdot_sim1, r_sim2, rdot_sim2, r_sim3, rdot_sim3, r_sim4, rdot_sim4,  magx, gyrox, magy, gyroy, magz, gyroz, accx, accy, accz, accT, gyroT, magT, time, i, j, k, m);
...
figure
subplot(311), plot(pos1)
subplot(312), plot(pos2)
subplot(313), plot(pos3)
%%
figure
...
rmsErrorX = rms(x(7,15:end)-PP3(15:end)-1.09)

figure
subplot(311), plot(time, x(1,:), 'LineWidth', 2); hold on; plot(time, PP1+1.03, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','NorthEast'); title('Position Along x Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([107.99, 111.984])
