...
% figure
% subplot(311), plot(time, PP1)
% subplot(312), plot(time, PP2)
% subplot(313), plot(time, PP3)
...
%
H1 = NaN(1,length(PP1)); H1_ = NaN(1,length(PP1)); r_sim1 = NaN(1,length(PP1)); r_sim1_ = NaN(1,length(PP1)); rdot_sim1 = NaN(1,length(PP1)); rdot_sim1_ = NaN(1,length(PP1)); 
H2 = NaN(1,length(PP1)); H2_ = NaN(1,length(PP1)); r_sim2 = NaN(1,length(PP1)); r_sim2_ = NaN(1,length(PP1)); rdot_sim2 = NaN(1,length(PP1)); rdot_sim2_ = NaN(1,length(PP1));
H3 = NaN(1,length(PP1)); H3_ = NaN(1,length(PP1)); r_sim3 = NaN(1,length(PP1)); r_sim3_ = NaN(1,length(PP1)); rdot_sim3 = NaN(1,length(PP1)); rdot_sim3_ = NaN(1,length(PP1)); 
H4 = NaN(1,length(PP1)); H4_ = NaN(1,length(PP1)); r_sim4 = NaN(1,length(PP1)); r_sim4_ = NaN(1,length(PP1)); rdot_sim4 = NaN(1,length(PP1)); rdot_sim4_ = NaN(1,length(PP1));

phi1 = zeros(1,length(PP1)); phi21 = NaN(1,length(PP1));  phi31 = zeros(1,length(PP1)); phi41 = NaN(1,length(PP1));
phi2 = zeros(1,length(PP1)); phi22 = NaN(1,length(PP1));  phi32 = zeros(1,length(PP1)); phi42 = NaN(1,length(PP1));
phi3 = zeros(1,length(PP1)); phi23 = NaN(1,length(PP1));  phi33 = zeros(1,length(PP1)); phi43 = NaN(1,length(PP1));
phi4 = zeros(1,length(PP1)); phi24 = NaN(1,length(PP1));  phi34 = zeros(1,length(PP1)); phi44 = NaN(1,length(PP1));
...
[H1(k+1),H1_(k+1),r_sim1(k+1),r_sim1_(k+1),rdot_sim1(k+1),rdot_sim1_(k+1),phi1(k+1), phi21(k+1), phi31(k+1), phi41(k+1), offset11, offset21, offset31, offset41] = noisysimFHopRecover(x1,f1,Gt,M,X,PT,GT,GR,R,sigma,1,k,z,z_prev,phi1(k),phi31(k),time(k+1)-time(k), offset11, offset21, offset31, offset41);
[H2(k+1),H2_(k+1),r_sim2(k+1),r_sim2_(k+1),rdot_sim2(k+1),rdot_sim2_(k+1),phi2(k+1), phi22(k+1), phi32(k+1), phi42(k+1), offset12, offset22, offset32, offset42] = noisysimFHopRecover(x2,f2,Gt,M,X,PT,GT,GR,R,sigma,2,k,z,z_prev,phi2(k),phi32(k),time(k+1)-time(k), offset12, offset22, offset32, offset42);
[H3(k+1),H3_(k+1),r_sim3(k+1),r_sim3_(k+1),rdot_sim3(k+1),rdot_sim3_(k+1),phi3(k+1), phi23(k+1), phi33(k+1), phi43(k+1), offset13, offset23, offset33, offset43] = noisysimFHopRecover(x3,f3,Gt,M,X,PT,GT,GR,R,sigma,3,k,z,z_prev,phi3(k),phi33(k),time(k+1)-time(k), offset13, offset23, offset33, offset43);   
[H4(k+1),H4_(k+1),r_sim4(k+1),r_sim4_(k+1),rdot_sim4(k+1),rdot_sim4_(k+1),phi4(k+1), phi24(k+1), phi34(k+1), phi44(k+1), offset14, offset24, offset34, offset44] = noisysimFHopRecover(x4,f4,Gt,M,X,PT,GT,GR,R,sigma,4,k,z,z_prev,phi4(k),phi34(k),time(k+1)-time(k), offset14, offset24, offset34, offset44);  
end
...
time1 = t1 +13.5167; time2 = t2 + 13.5867; time3 = t3+13.1267;
...
subplot(412), plot(time1,  measr2, 'LineWidth', 1), hold on, plot(time, r_sim2, 'LineWidth', 1),title('Radial Distance $r2$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([107.99, 111.984]);
subplot(413), plot(time3,  measr3, 'LineWidth', 1), hold on, plot(time, r_sim3, 'LineWidth', 1),title('Radial Distance $r3$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([107.99, 111.984]);
subplot(414), plot(time1,  measr4, 'LineWidth', 1), hold on, plot(time, r_sim4, 'LineWidth', 1),title('Radial Distance $r4$','interpreter','latex'); xlabel('time[s]'), ylabel('Distance [m]','interpreter','latex'), legend('measurement','ground truth'), xlim([107.99, 111.984]);
...
x = [0.5;0.5;0.5; 0.5;0.5;0.5; 0.5;0.5;0.5; 0.5;0.5;0.5; 1.03;0;0.01]*ones(1,length(time)-N);e = NaN(1,length(time)-N);
    
for m = 1:1:length(time)-N
    
    [y, len] = gety(r_sim1, rdot_sim1, r_sim2, rdot_sim2, r_sim3, rdot_sim3, r_sim4, rdot_sim4, m, N);

    x(:,m) = lsqnonlin(@(xx)getNLE(y, xx, N, len, time, m),[0.5;0.5;0.5; 0.5;0.5;0.5; 0.5;0.5;0.5; 0.5;0.5;0.5; 0.5;0.5;0.5]);
...
figure
subplot(311), plot(time(1:end-N), x(1,:), 'LineWidth', 2); hold on; plot(time, PP1+1.03, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','NorthEast'); title('Position Along x Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([107.99, 111.984])
subplot(312), plot(time(1:end-N), x(2,:), 'LineWidth', 2); hold on; plot(time, VV1, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth','Location','SouthEast'); title('Velocity Along x Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([107.99, 111.984])%xlim([3.5, 7.5])
...
figure
subplot(311), plot(time(1:end-N), x(4,:), 'LineWidth', 2); hold on; plot(time, PP2+1.31, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','NorthEast'); title('Position Along y Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([107.99, 111.984])
subplot(312), plot(time(1:end-N), x(5,:), 'LineWidth', 2); hold on; plot(time, VV2, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth','Location','NorthEast'); title('Velocity Along y Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([107.99, 111.984])%xlim([3.5, 7.5])
...
figure
subplot(311), plot(time(1:end-N), x(7,:), 'LineWidth', 2); hold on; plot(time, PP3+1.03, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','SouthEast'); title('Position Along z Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([107.99, 111.984])
subplot(312), plot(time(1:end-N), x(8,:), 'LineWidth', 2); hold on; plot(time, VV3, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth'); title('Velocity Along z Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([107.99, 111.984])%xlim([3.5, 7.5])
...
[pos2, vel2] = doubleInte(diff(time(1:end-N)), -x(9,2:end));
[pos3, vel3] = doubleInte(diff(time(1:end-N)), x(6,2:end));
...
% subplot(312), plot(time(1:end-N-1), pos2, 'LineWidth', 2); hold on; plot(time, PP2, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth'); title('Velocity Along x Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([108.017, 111.984])
% subplot(313), plot(time(1:end-N-1), pos3, 'LineWidth', 2); hold on; plot(time, PP3, 'LineWidth', 2); legend('Estimated Acceleration','Ground Truth'); title('Acceleration Along x Axis [m/s^2]'); xlabel('t [s]'); ylabel('Acceleration [m/s^2]'); grid on; grid minor;xlim([108.017, 111.984])
% 
% 
% figure
% subplot(311), plot(time(1:end-N-1), pos1, 'LineWidth', 3); hold on; plot(time, PP1, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','SouthEast'); title('Position Along x Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([108.017, 111.984])
...
% subplot(313), plot(time(1:end-N-1), -x(9,2:end), 'LineWidth', 3); hold on; plot(time, AA2, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','SouthEast'); title('Acceleration Along y Axis [m]'); xlabel('t [s]'); ylabel('Acceleration [m/s^2]'); grid on; grid minor;xlim([108.017, 111.984])
% 
% figure
...
% subplot(313), plot(time(1:end-N-1), x(6,2:end), 'LineWidth', 3); hold on; plot(time, AA3, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','SouthEast'); title('Acceleration Along z Axis [m]'); xlabel('t [s]'); ylabel('Acceleration [m/s^2]'); grid on; grid minor;xlim([108.017, 111.984])
% 

%%
rmsErrorX = rms(x(1,:)-PP1(1:end-N)-1.03)
rmsErrorY = rms(x(4,:)-PP2(1:end-N)-1.31)
rmsErrorZ = rms(x(7,:)-PP3(1:end-N)-1.09)
%%
%
% figure
