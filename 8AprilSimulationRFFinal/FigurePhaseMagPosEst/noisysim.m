function [v, v2, r, r2, rdot, rdot2, phase, phi_mod_gt, phi_mod2_gt, phi_mod, phi_mod2, offset1, offset2, offset3, offset4] = noisysim(x,f,Gt,M,X,PT,GT,GR,R,sigma,index,k,z,z_prev,phi_prev_mod_gt_load, phi_prev_mod_load,T,offset1, offset2, offset3, offset4)
% ============================================= Magnitude =================================================
...
H  = abs(sqrt(2*R*PT*GT*GR*Gt^2*lambda^2*X^2*M)/(4*pi)*(mul+ 0.2*mulRef1 + 0.2*mulRef2 + 0.2*mulRef3 + 0.2*mulRef4 + 0.2*mulRef5 +0.2*mulRef6));%  + 0.6*mulSum3 %mul+ 0.25* mulSum + 0.25*mulSum2
H2 = abs(sqrt(2*R*PT*GT*GR*Gt^2*lambda^2*X^2*M)/(4*pi)*(mul));%mul+ 0.25* mulSum + 0.25*mulSum2

% ++++++++++++++++++++++++++++++++++++++++++ Noise of Magnitude +++++++++++++++++++++++++++++++++++++++++++

pd = makedist('Rician','s',sqrt(H),'sigma',0.005*sqrt(H));
...
r  = sqrt(2*PT*GT*GR*Gt^2*lambda^2*X^2*M*R)/(4*pi*v);
r2 = sqrt(2*PT*GT*GR*Gt^2*lambda^2*X^2*M*R)/(4*pi*H2);
%%

% % ============================================== Phase ====================================================

phi_prev_mod  = angle(sqrt(2*R*PT*GT*GR*Gt^2*lambda^2*X^2*M)/(4*pi)*(mul_prev   + 0.2*mulRef1_prev + 0.2*mulRef2_prev + 0.2*mulRef3_prev + 0.2*mulRef4_prev + 0.2*mulRef5_prev + 0.2*mulRef6_prev));%mod(4*pi*sqrt((z_prev(1,k)-x(1))^2+(z_prev(2,k)-x(2))^2)/lambda,2*pi);
phi_prev_mod2 = angle(sqrt(2*R*PT*GT*GR*Gt^2*lambda2^2*X^2*M)/(4*pi)*(mul_prev2 + 0.2*mulRef12_prev+0.2*mulRef22_prev+0.2*mulRef32_prev+0.2*mulRef42_prev+0.2*mulRef52_prev+0.2*mulRef62_prev));%mod(4*pi*sqrt((z_prev(1,k)-x(1))^2+(z_prev(2,k)-x(2))^2)/lambda,2*pi);
...
phi_mod      = angle(sqrt(2*R*PT*GT*GR*Gt^2*lambda^2*X^2*M)/(4*pi)*(mul + 0.2*mulRef1 + 0.2*mulRef2 + 0.2*mulRef3+0.2*mulRef4+0.2*mulRef5+0.2*mulRef6));%4*pi*sqrt((z(1,k)-x(1))^2+(z(2,k)-x(2))^2+(z(3,k)-x(3))^2)/lambda; %4*pi*sqrt((z(1,k)-x(1))^2+(z(2,k)-x(2))^2+(z(3,k)-x(3))^2)/lambda;%mod(4*pi*sqrt((z(1,k)     -x(1))^2+(z(2,k)     -x(2))^2)/lambda,2*pi);
phi_mod2     = angle(sqrt(2*R*PT*GT*GR*Gt^2*lambda2^2*X^2*M)/(4*pi)*(mul2 + 0.2*mulRef12+0.2*mulRef22+0.2*mulRef32+0.2*mulRef42+0.2*mulRef52+0.2*mulRef62));%4*pi*sqrt((z(1,k)-x(1))^2+(z(2,k)-x(2))^2+(z(3,k)-x(3))^2)/lambda2;%mod(4*pi*sqrt((z(1,k)     -x(1))^2+(z(2,k)     -x(2))^2)/lambda,2*pi);
phase = phi_mod2;
% Angle
...
rdot2 = 3*10^8/(4*pi)*(phi_mod -phi_mod2)/(0.1*10^9);%lambda/(4*pi)*diff*1/T;
