function [v, v2, r, r2, rdot, phi_mod] = noisysim33(x,f,Gt,M,X,PT,GT,GR,R,sigma,index,k,z,z_prev,T,time1,multi)
...
phi_mod = angle(sqrt((2*R*PT*GT*GR*Gt^2*lambda^4*X^2*M)/(4*pi)^4*(mul+ 0.9*mulSum3 + 0.2*mulSum4 + 0.2*mulSum5 + 0.8*mulSum6 + 0.1*mulSum7 +0.2*mulSum8)^4));%  + 0.6*mulSum3 %mul+ 0.25* mulSum + 0.25*mulSum2
