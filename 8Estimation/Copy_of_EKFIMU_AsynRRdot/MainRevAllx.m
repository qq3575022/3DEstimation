...
% start time
xS = find(abs(acc_time-44.1809)<0.002); xS = xS(1);
xE = find(abs(acc_time-47.6236)<0.001); xE = xE(1);

xxS = find(abs(gyro_time-44.1809)<0.002); xxS = xxS(1);
xxE = find(abs(gyro_time-47.6236)<0.001); xxE = xxE(1);

xSS = find(abs(mag_time-44.1809)<0.009); xSS = xSS(1);
xEE = find(abs(mag_time-47.6236)<0.012); xEE = xEE(1);
...
timeAll = sort(unique([accT; gyroT; magT],'rows'));

% indexA = 1;
% indexG = 1;
% indexM = 1;
% 
% for i = 1:1:length(timeAll)
%     if timeAll(i) == accT(indexA)
%         indexA= indexA + 1;
%     end
%     if timeAll(i) == gyroT(indexG)
%         indexG= indexG + 1;
%     end
%     if timeAll(i) == magT(indexM)
%         indexM= indexM + 1;
%     end
%     if timeAll(i) ~= accT(indexA) && timeAll(i) ~= gyroT(indexG) && timeAll(i) ~= magT(indexM)
%         i
%         timeAll(i)
%         accT(indexA)
%         gyroT(indexG)
...
[PP1, VV1, AA1] = groundtruth1Dx2(timeAll - timeAll(1));
...
j = 1;
k = 1;
for m = 1:1:length(timeAll)
    
    %[index, i, j, k] = getIndex(i, j, k, mag_time, gyro_time, acc_time, m, time);
 
    [y, i, j, k, index] = getyNPVA(magx, gyrox, magy, gyroy, magz, gyroz, accx, accy, accz, accT, gyroT, magT, timeAll, i, j, k, m);
...
figure
subplot(311), plot(timeAll, x(1,:), 'LineWidth', 3); hold on; plot(timeAll, PP1, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','SouthEast'); title('Position Along x Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([44.1809, 47.6236])
subplot(312), plot(timeAll, x(2,:), 'LineWidth', 2); hold on; plot(timeAll, VV1, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth'); title('Velocity Along x Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([44.1809, 47.6236])
subplot(313), plot(timeAll, x(3,:), 'LineWidth', 2); hold on; plot(timeAll, AA1, 'LineWidth', 2); legend('Estimated Acceleration','Ground Truth'); title('Acceleration Along x Axis [m/s^2]'); xlabel('t [s]'); ylabel('Acceleration [m/s^2]'); grid on; grid minor;xlim([44.1809, 47.6236])

rmsErrorX = rms(x(1,:)-PP1)
%%
figure
subplot(9,1,1), plot(timeAll, x(1,:), 'b', 'LineWidth', 2); %hold on; plot(time, XX(3,:), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated acceleration along x axis'); grid on;
subplot(9,1,2), plot(timeAll, x(2,:), 'b', 'LineWidth', 2); %hold on; plot(time, XX(6,:), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated acceleration along y axis'); grid on;
subplot(9,1,3), plot(timeAll, x(3,:), 'b', 'LineWidth', 2); %hold on; plot(time, XX(9,:), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated acceleration along z axis'); grid on;
subplot(9,1,4), plot(timeAll, x(4,:), 'b', 'LineWidth', 2); %hold on; plot(time,, phi_gt(1,:), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated orientation along x axis'); grid on;
subplot(9,1,5), plot(timeAll, x(5,:), 'b', 'LineWidth', 2); %hold on; plot(time, phi_gt(2,:), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated angular velocity along x axis'); grid on;
subplot(9,1,6), plot(timeAll, x(6,:), 'b', 'LineWidth', 2); %hold on; plot(td3, phi_gt(3,:), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated orientation along y axis'); grid on;
subplot(9,1,7), plot(timeAll, x(7,:), 'b', 'LineWidth', 2); %hold on; plot(td3, phi_gt(4,:), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated angular velocity along y axis'); grid on;
subplot(9,1,8), plot(timeAll, x(8,:), 'b', 'LineWidth', 2); %hold on; plot(td3, phi_gt(5,:), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated orientation along z axis'); grid on;
subplot(9,1,9), plot(timeAll, x(9,:), 'b', 'LineWidth', 2); %hold on; plot(td3, phi_gt(6,:), 'r', 'LineWidth', 2); legend('estimated', 'gt'); title('Estimated angular velocity along z axis'); grid on;
