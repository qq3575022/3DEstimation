...
zzS = find(abs(gyro_time-51.7958)<0.002); zzS = zzS(1);
zzE = find(abs(gyro_time-53.916)<0.002); zzE = zzE(1);

zSS = find(abs(mag_time-51.7958)<0.009); zSS = zSS(1);
zEE = find(abs(mag_time-53.916)<0.009); zEE = zEE(1);
...
figure
subplot(311), plot(time, x(4,:), 'LineWidth', 3); hold on; plot(time, PP1, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','SouthEast'); title('Position Along z Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([51.7958, 53.916]);
subplot(312), plot(time, x(5,:), 'LineWidth', 2); hold on; plot(time, VV1, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth','Location','NorthEast'); title('Velocity Along z Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([51.7958, 53.916]);
subplot(313), plot(time, x(6,:), 'LineWidth', 2); hold on; plot(time, AA1, 'LineWidth', 2); legend('Estimated Acceleration','Ground Truth','Location','SouthEast'); title('Acceleration Along z Axis [m/s^2]'); xlabel('t [s]'); ylabel('Acceleration [m/s^2]'); grid on; grid minor;xlim([51.7958, 53.916]);
