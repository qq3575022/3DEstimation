...
% start time
xS = find(abs(acc_time-32.58)<0.002); xS = xS(1);
xE = find(abs(acc_time-36.608)<0.002); xE = xE(1);

xxS = find(abs(gyro_time-32.58)<0.002); xxS = xxS(1);
xxE = find(abs(gyro_time-36.608)<0.002); xxE = xxE(1);

xSS = find(abs(mag_time-32.58)<0.009); xSS = xSS(1);
xEE = find(abs(mag_time-36.608)<0.012); xEE = xEE(1);
...
accx = acc_data_x(xS:xE) - mean(acc_data_x(xS:xE))+0.007;
...
figure
subplot(311), plot(time, x(1,:), 'LineWidth', 3); hold on; plot(time, PP1, 'LineWidth', 2); legend('Estimated Position','Ground Truth','Location','SouthEast'); title('Position Along x Axis [m]'); xlabel('t [s]'); ylabel('Position [m]'); grid on; grid minor;xlim([32.608, 36.608])
subplot(312), plot(time, x(2,:), 'LineWidth', 2); hold on; plot(time, VV1, 'LineWidth', 2); legend('Estimated Velocity','Ground Truth'); title('Velocity Along x Axis [m/s]'); xlabel('t [s]'); ylabel('Velocity [m/s]'); grid on; grid minor;xlim([32.608, 36.608])
subplot(313), plot(time, x(3,:), 'LineWidth', 2); hold on; plot(time, AA1, 'LineWidth', 2); legend('Estimated Acceleration','Ground Truth'); title('Acceleration Along x Axis [m/s^2]'); xlabel('t [s]'); ylabel('Acceleration [m/s^2]'); grid on; grid minor;xlim([32.608, 36.608])
